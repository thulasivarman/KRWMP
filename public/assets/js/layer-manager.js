    ...
    if (isPointLayer(layer) && layer.fill_layer_id && !window.KRWMP_MAP.getLayer(layer.fill_layer_id)) {
        window.KRWMP_MAP.addLayer({
            ...
            paint: {
                'circle-radius': [
                    'match',
                    ['downcase', ['coalesce', ['get', 'severity_level'], 'medium']],
                    'high', 9,
                    'medium', 7,
                    'low', 6,
                    7
...
function getLegendSymbol(layer) {
    if (layer.layer_key === 'community_complaints') {
        return '<span class="inline-flex gap-0.5"><i class="h-2.5 w-2.5 rounded-full bg-red-500 border border-white/70"></i><i class="h-2.5 w-2.5 rounded-full bg-amber-500 border border-white/70"></i><i class="h-2.5 w-2.5 rounded-full bg-green-500 border border-white/70"></i></span>';
    }
