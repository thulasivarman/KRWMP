...
function renderIntervention(item) {
  ...
  card.innerHTML = `<div class="flex justify-between gap-4"><div><h3 class="font-bold text-slate-100">${escapeHtml(item.intervention_title)} (${escapeHtml(item.intervention_code)})</h3><p class="text-xs text-slate-500">${escapeHtml(item.library_name || '-')} · ${escapeHtml(item.status || '-')} · ${escapeHtml(item.priority || '-')} · ${escapeHtml(item.dsd_name || '-')} / ${escapeHtml(item.gnd_name || '-')}</p><p class="text-[10px] text-slate-600">Updated by ${escapeHtml(item.updated_by || '-')} on ${formatDate(item.updated_at)}</p></div><div class="manage-actions hidden krwmp-table-actions"><button data-edit="${item.id}"  class="krwmp-btn krwmp-btn-secondary krwmp-btn-sm">Edit</button><button data-delete="${item.id}"  class="krwmp-btn krwmp-btn-danger krwmp-btn-sm">Delete</button></div></div><div><div class="flex justify-between text-xs text-slate-400 mb-1"><span>Progress</span><span class="krwmp-badge krwmp-badge-success">${progress}%</span></div><div class="h-2 rounded bg-slate-800 overflow-hidden"><div class="h-full bg-emerald-500" style="width:${Math.max(0, Math.min(100, progress))}%"></div></div></div><div class="grid grid-cols-1 lg:grid-cols-2 gap-4"><div><h4 class="krwmp-section-title mb-2">Action Timeline & Responsible Officers</h4><div class="space-y-3">${actionOfficerCardsHtml(item.timeline || [], item.officers || [])}</div></div><div class="manage-actions hidden"><form class="action-officer-form krwmp-card-muted p-3 space-y-2"><h4 class="krwmp-section-title">Action & Responsible Officer</h4><input type="date" name="action_date"  class="krwmp-input"><input name="action_title" required placeholder="Action title"  class="krwmp-input"><textarea name="action_description" placeholder="Action details"  class="krwmp-textarea"></textarea><input name="progress_percent" type="number" min="0" max="100" placeholder="Progress %"  class="krwmp-input"><input name="officer_name" required placeholder="Responsible officer name"  class="krwmp-input"><input name="designation" placeholder="Designation"  class="krwmp-input"><select name="institution"  class="krwmp-select officer-institution-select">${institutionOptions()}</select><input name="officer_contact" placeholder="Officer contact number"  class="krwmp-input"><input name="responsibility" placeholder="Responsibility for this action"  class="krwmp-input"><button  class="krwmp-btn krwmp-btn-primary krwmp-btn-full">Save Action & Officer</button></form></div></div>`;
...
function actionOfficerCardsHtml(timeline, officers) {
  if (!timeline.length && !officers.length) return '<div class="krwmp-empty-state">No actions or responsible officers recorded.</div>';
  ...
  const actionCards = (timeline || []).map(t => {
    ...
    return `<div class="krwmp-card-muted p-3 text-xs space-y-2"><div class="flex items-start justify-between gap-3"><div><strong class="text-slate-100">${escapeHtml(t.action_title)}</strong><div class="text-slate-500">${formatDate(t.action_date)} · ${escapeHtml(t.action_status || 'completed')} · ${escapeHtml(t.progress_percent ?? '')}%</div></div><span class="krwmp-badge krwmp-badge-success">${escapeHtml(t.progress_percent ?? 0)}%</span></div><p class="text-slate-300">${escapeHtml(t.action_description || '-')}</p><div class="border-t border-slate-800 pt-2"><div class="font-bold text-emerald-400">Responsible Officer</div><div class="text-slate-200">${escapeHtml(t.officer_name || officer.officer_name || '-')}</div><div class="text-slate-500">${escapeHtml(officer.designation || '-')} · ${escapeHtml(officer.institution || '-')}</div><div class="text-slate-500">${escapeHtml(t.officer_contact || officer.phone || '-')} · ${escapeHtml(officer.responsibility || '-')}</div></div></div>`;
  }).join('');
  const unmatchedOfficerCards = (officers || []).filter(o => !(timeline || []).some(t => String(t.officer_name || '').toLowerCase() === String(o.officer_name || '').toLowerCase())).map(o => `<div class="krwmp-card-muted p-3 text-xs"><div class="font-bold text-emerald-400">Responsible Officer</div><div class="text-slate-200">${escapeHtml(o.officer_name || '-')}</div><div class="text-slate-500">${escapeHtml(o.designation || '-')} · ${escapeHtml(o.institution || '-')}</div><div class="text-slate-500">${escapeHtml(o.phone || '-')} · ${escapeHtml(o.responsibility || '-')}</div></div>`).join('');
