...
async function saveActionWithOfficer(e, id) {
  ...
  try {
    await json(`/api/interventions/registry/${id}/timeline`, { method: 'POST', headers: headers({ 'Content-Type': 'application/json' }), body: JSON.stringify({ action_date: body.action_date, action_title: body.action_title, action_description: body.action_description, progress_percent: body.progress_percent, officer_name: officerName, officer_contact: officerContact, action_status: 'completed' }) });
    await json(`/api/interventions/registry/${id}/officers`, { method: 'POST', headers: headers({ 'Content-Type': 'application/json' }), body: JSON.stringify({ officer_name: officerName, designation: body.designation, institution: body.institution, phone: officerContact, responsibility: body.responsibility || body.action_title }) });
...
async function deleteIntervention(id) {
  if (!canManage) return show('Only officer/admin users can delete interventions.', true);
