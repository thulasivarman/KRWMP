...
function ensureSpatialFields() {
  ...
  if (picker && !document.getElementById('autoSpatialDisplay')) {
    ...
    box.className = 'grid grid-cols-1 md:grid-cols-3 gap-3 text-xs mt-3 border-t border-slate-800 pt-3';
    box.innerHTML = '<div class="bg-slate-900/80 border border-slate-800 rounded p-3"><div class="text-slate-500 uppercase tracking-wider font-bold">DS Division</div><div id="autoDsdText" class="text-slate-200 mt-1">Not detected</div></div><div class="bg-slate-900/80 border border-slate-800 rounded p-3"><div class="text-slate-500 uppercase tracking-wider font-bold">GN Division</div><div id="autoGndText" class="text-slate-200 mt-1">Not detected</div></div><div class="bg-slate-900/80 border border-slate-800 rounded p-3"><div class="text-slate-500 uppercase tracking-wider font-bold">Sub Watershed</div><div id="autoSubText" class="text-slate-200 mt-1">Pending upload</div></div>';
...
async function loadCommittees() { committeeList.innerHTML = '<p class="text-sm text-slate-400">Loading VWMC records...</p>'; const data = await json('/api/vwmc/committees'); committees = data.committees || []; if (currentPage > totalPages()) currentPage = totalPages(); renderCommittees(); }
