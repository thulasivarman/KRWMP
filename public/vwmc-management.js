...
function validateCommitteeForm() {
  ...
  if (!committeeForm.dsd_name.value) errors.push('Please select a DSD.');
  if (!committeeForm.gnd_name.value) errors.push('Please select a GND.');
...
committeeForm.addEventListener('submit', async e => { e.preventDefault(); if (!canManage) return showStatus('You have view-only permission.', true); if (!validateCommitteeForm()) return; const body = Object.fromEntries(new FormData(committeeForm)); const id = body.id; delete body.id; try { if (id) { await json(`/api/vwmc/committees/${id}`, { method: 'PUT', headers: headers({ 'Content-Type': 'application/json' }), body: JSON.stringify(body) }); showStatus('VWMC updated successfully.'); } else { await json('/api/vwmc/committees', { method: 'POST', headers: headers({ 'Content-Type': 'application/json' }), body: JSON.stringify(body) }); showStatus('VWMC created successfully.'); } committeeForm.reset(); vwmcLocationPicker?.clear(); await loadGnds(''); await loadCommittees(); } catch (error) { showStatus(error.message, true); } });
async function saveMember(event, committeeId) { event.preventDefault(); if (!canManage) return showStatus('You have view-only permission.', true); const form = event.target; if (!validateMemberForm(form)) return; const body = Object.fromEntries(new FormData(form)); delete body.committee_id; const editMemberId = form.dataset.editMemberId; try { if (editMemberId) { await json(`/api/vwmc/members/${editMemberId}`, { method: 'PUT', headers: headers({ 'Content-Type': 'application/json' }), body: JSON.stringify(body) }); showStatus('Member updated successfully.'); } else { await json(`/api/vwmc/committees/${committeeId}/members`, { method: 'POST', headers: headers({ 'Content-Type': 'application/json' }), body: JSON.stringify(body) }); showStatus('Member added successfully.'); } form.reset(); form.dataset.editMemberId = ''; await loadCommittees(); } catch (error) { showStatus(error.message, true); } }
...
(async () => { await initSidebar(); vwmcLocationPicker = new KRWMPLocationPicker({ containerId: 'vwmcLocationPicker', latitudeInput: '#latitudeInput', longitudeInput: '#longitudeInput' }); await loadDsds(); await loadGnds(''); applyPermissions(); await loadCommittees(); })().catch(e => showStatus(e.message, true));
