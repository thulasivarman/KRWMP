...
const reporterEmailInput = document.getElementById('reporterEmailInput');
...
function prefillReporterFromSession() { const user = currentUser(); const displayName = user.name || user.full_name || user.username || user.identifier || ''; const contact = user.contact_number || user.phone || user.phone_number || user.mobile || user.telephone || ''; const email = user.email || ''; if (displayName && !reporterNameInput.value) reporterNameInput.value = displayName; if (contact && !reporterContactInput.value) reporterContactInput.value = contact; if (email && reporterEmailInput && !reporterEmailInput.value) reporterEmailInput.value = email; }
