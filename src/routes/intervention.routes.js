...
async function interventionRoutes(fastify) {
  ...
  fastify.get('/interventions/lookups/institutions', async (request, reply) => {
    if (!await requirePrivilegeInline(request, reply, 'intervention_registry_view', 'view')) return;
    const result = await pool.query(`SELECT id, institution_name, institution_type, contact_person, contact_phone, contact_email, active FROM public.intervention_institutions WHERE active = true ORDER BY institution_name;`);
