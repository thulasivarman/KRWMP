...
async function vectorLayerRoutes(fastify) {
  ...
  fastify.post('/vector-layers/upload', async (request, reply) => {
    ...
    if (!data) {
      return reply.status(400).send({
        success: false,
        message: 'No GeoJSON file uploaded',
      });
    }
    ...
    const payloadFields = {
      ...
      color: getFieldValue(fields, 'color'),
      weight: getFieldValue(fields, 'weight'),
      opacity: getFieldValue(fields, 'opacity'),
      fillColor: getFieldValue(fields, 'fillColor'),
      fillOpacity: getFieldValue(fields, 'fillOpacity'),
      radius: getFieldValue(fields, 'radius'),
      popupFields: getFieldValue(fields, 'popupFields'),
    };
    ...
    return {
      success: true,
      message: 'GeoJSON imported into Supabase/PostGIS successfully',
      layer,
    };
  });
  ...
  fastify.delete('/vector-layers/:id', async (request, reply) => {
    ...
    return {
      success: true,
      message: 'Layer table and registry record deleted successfully',
