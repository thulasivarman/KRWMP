...
function normalizeSymbology(value) {
  ...
  const classes = Array.isArray(parsed.classes) ? parsed.classes.slice(0, MAX_RASTER_CLASSES).map((item, index) => {
    ...
    return {
      ...
      color: /^#[0-9a-f]{6}$/i.test(item.color || '') ? item.color : defaultClassColor(index),
      label: String(item.label || `${Math.min(min, max)} - ${Math.max(min, max)}`).slice(0, 80)
    };
  }).filter(Boolean) : [];
  return { mode, classes };
}
...
async function updateRasterLayer(layerKey, body = {}) {
  ...
  if (symbologyProvided) {
    ...
    if (!fs.existsSync(inputPath)) throw new Error('Original GeoTIFF file not found on server. Re-upload the raster before applying classified symbology.');
