...
function normalizeStatus(value, fallback = 'pending') {
  ...
  if (!VALID_STATUSES.has(status)) throw new Error('Invalid Knowledge Resource status.');
  return status;
}
...
async function createContent(body = {}, user = 'system') {
  ...
  try {
    ...
    const result = await client.query(`
      ...
      RETURNING id;
    `, [title, cleanText(body.summary), source.contentType, body.category_id || null, body.publisher_institution_id || null, body.author_institution_id || null, body.publication_year ? Number(body.publication_year) : null, cleanText(body.language) || 'English', cleanText(body.keywords), cleanText(body.abstract), cleanText(body.body_content), source.externalUrl, source.fileUrl, cleanText(body.thumbnail_url), source.videoUrl, status, body.is_featured, latitude, longitude, derived.dsd_name, derived.gnd_name, derived.sub_watershed_id, derived.sub_watershed_name, user]);
...
async function updateContent(id, body = {}, user = 'system') {
  ...
  validateCoordinates(latitude, longitude);
  const derived = await deriveLocation(latitude, longitude);
  const status = body.status ? normalizeStatus(body.status, existing.status) : existing.status;
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    await client.query(`
      UPDATE public.knowledge_content
      SET title = COALESCE($2, title), summary = COALESCE($3, summary), content_type = $4, category_id = COALESCE($5::uuid, category_id), publisher_institution_id = $6::uuid, author_institution_id = $7::uuid, publication_year = $8::integer, language = COALESCE($9, language), keywords = $10, abstract = $11, body_content = $12, external_url = $13, file_url = $14, thumbnail_url = $15, video_url = $16, status = $17, is_featured = COALESCE($18::boolean, is_featured), geom = CASE WHEN $19::double precision IS NULL OR $20::double precision IS NULL THEN NULL ELSE ST_SetSRID(ST_MakePoint($20::double precision,$19::double precision),4326) END, dsd_name = $21, gnd_name = $22, sub_watershed_id = $23::uuid, sub_watershed_name = $24, updated_by = $25, updated_at = now(), published_at = CASE WHEN $17 = 'published' AND published_at IS NULL THEN now() ELSE published_at END, published_by = CASE WHEN $17 = 'published' AND published_by IS NULL THEN $25 ELSE published_by END
      WHERE id = $1 AND COALESCE(is_deleted, false) = false;
    `, [id, cleanText(body.title), cleanText(body.summary), source.contentType, body.category_id || existing.category_id || null, body.publisher_institution_id || null, body.author_institution_id || null, body.publication_year ? Number(body.publication_year) : existing.publication_year || null, cleanText(body.language), cleanText(body.keywords), cleanText(body.abstract), cleanText(body.body_content), source.externalUrl, source.fileUrl, cleanText(body.thumbnail_url), source.videoUrl, status, body.is_featured, latitude, longitude, derived.dsd_name, derived.gnd_name, derived.sub_watershed_id, derived.sub_watershed_name, user]);
...
async function updateContentStatus(id, statusValue, user = 'system', reviewRemarks = null) {
  const status = normalizeStatus(statusValue);
  const result = await pool.query(`
    UPDATE public.knowledge_content
    SET status = $2,
        review_remarks = COALESCE($4, review_remarks),
        updated_by = $3,
        updated_at = now(),
        reviewed_by = CASE WHEN $2 IN ('reviewed','rejected','published') THEN $3 ELSE reviewed_by END,
...
module.exports = { listCategories, createCategory, updateCategory, listTags, ensureTags, listContent, getContent, createContent, updateContent, updateContentStatus, deleteContent, dashboard, geoJson, report };
