...
let personLocationColumnCache = null;
...
function toNumber(value) { if (value === undefined || value === null || String(value).trim() === '') return null; const n = Number(value); return Number.isFinite(n) ? n : null; }
function initialsFromName(name = '') { return String(name || '').split(/\s+/).map(part => part[0]).filter(Boolean).join('').slice(0, 4).toUpperCase() || 'USR'; }

async function hasPersonLocationColumns() {
  if (personLocationColumnCache !== null) return personLocationColumnCache;
  ...
  `);
  personLocationColumnCache = Number(result.rows[0]?.count || 0) === 2;
  return personLocationColumnCache;
}

function validateProfileLocation(latitude, longitude, supportsLocationColumns = true) {
  if (!supportsLocationColumns) return;
  if (latitude === null && longitude === null) return;
  if (latitude === null || longitude === null) { const error = new Error('Both latitude and longitude are required when selecting profile location.'); error.statusCode = 400; throw error; }
  if (latitude < 5 || latitude > 10 || longitude < 79 || longitude > 82) { const error = new Error('Profile location must be within the Sri Lanka coordinate range.'); error.statusCode = 400; throw error; }
}
...
function validateSelfProfile(body = {}) {
  ...
  if (!name || name.length < 2) { const error = new Error('Full name is required.'); error.statusCode = 400; throw error; }
  if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { const error = new Error('Email format is invalid.'); error.statusCode = 400; throw error; }
  if (phoneNumber && !/^[0-9+()\-\s]{7,30}$/.test(phoneNumber)) { const error = new Error('Phone number format is invalid.'); error.statusCode = 400; throw error; }
...
async function getProfile(identifier) {
  const supportsLocationColumns = await hasPersonLocationColumns();
  const locationSelect = supportsLocationColumns ? 'p.latitude, p.longitude,' : 'NULL::numeric AS latitude, NULL::numeric AS longitude,';
  const query = `
...
async function updateProfile(identifier, data) {
  ...
  await pool.query(`UPDATE public.users SET name = $1, initials = $2, email = $3, phone_number = $4 WHERE identifier = $5;`, [name, initials.toUpperCase(), email, phoneNumber, identifier.trim().toLowerCase()]);
}
...
async function getSelfProfile(identifier) {
  ...
  return {
    user: { id: profile.id, name: profile.name, designation: profile.designation, initials: profile.initials, identifier: profile.identifier, email: profile.email, phone_number: profile.phone_number, institution_id: profile.institution_id, institution_name: profile.institution_name, role_name: profile.role_name, visible_sections: profile.visible_sections },
    person: profile.person_id ? { id: profile.person_id, full_name: profile.person_full_name, preferred_name: profile.preferred_name, nic_number: profile.nic_number, gender: profile.gender, date_of_birth: profile.date_of_birth, phone_number: profile.person_phone_number, email: profile.person_email, address: profile.address, dsd: profile.dsd, gnd: profile.gnd, latitude: profile.latitude, longitude: profile.longitude, status: profile.person_status } : null,
  };
}
...
async function updateSelfProfile(identifier, body = {}) {
  ...
  validateProfileLocation(latitude, longitude, supportsLocationColumns);
  ...
  try {
    ...
    const userResult = await client.query(`
      UPDATE public.users
      SET name = $2, initials = $3, email = $4, phone_number = $5
      WHERE identifier = $1
    ...
    if (!userResult.rows.length) { const error = new Error('Profile not found.'); error.statusCode = 404; throw error; }
    const linkedUserId = String(userResult.rows[0].id);
    ...
    if (personResult.rows.length) {
      if (supportsLocationColumns) {
        await client.query(`
          UPDATE public.persons
          SET full_name = $2, preferred_name = $3, phone_number = $4, email = $5, address = $6, dsd = $7, gnd = $8, latitude = $9, longitude = $10, updated_at = now()
          WHERE id = $1;
        `, [personResult.rows[0].id, name, preferredName, phoneNumber, email, address, dsd, gnd, latitude, longitude]);
      } else {
        await client.query(`
          UPDATE public.persons
          SET full_name = $2, preferred_name = $3, phone_number = $4, email = $5, address = $6, dsd = $7, gnd = $8, updated_at = now()
          WHERE id = $1;
        `, [personResult.rows[0].id, name, preferredName, phoneNumber, email, address, dsd, gnd]);
      }
    } else if (supportsLocationColumns) {
      await client.query(`
...
async function updateSelfPassword(identifier, body = {}) {
  ...
  if (!currentPassword || !newPassword || !confirmPassword) { const error = new Error('Current password, new password and confirmation are required.'); error.statusCode = 400; throw error; }
  if (newPassword.length < 8) { const error = new Error('New password must contain at least 8 characters.'); error.statusCode = 400; throw error; }
  if (newPassword !== confirmPassword) { const error = new Error('New password and confirmation do not match.'); error.statusCode = 400; throw error; }
  ...
  if (!result.rows.length) { const error = new Error('Profile not found.'); error.statusCode = 404; throw error; }
  const passwordMatch = await bcrypt.compare(currentPassword, result.rows[0].password_hash);
  if (!passwordMatch) { const error = new Error('Current password is incorrect.'); error.statusCode = 400; throw error; }
