...
async function login(username, password) {
  ...
  const query = `
    SELECT u.id, u.name, u.designation, u.initials, u.identifier, u.password_hash,
...
async function getProfile(identifier) {
  const query = `
    SELECT u.id, u.name, u.designation, u.initials, u.identifier, r.role_name, r.visible_sections
    FROM public.users u
    LEFT JOIN public.roles r ON u.role_id = r.id
    WHERE u.identifier = $1
    LIMIT 1;
...
async function updateProfile(identifier, data) {
  const { name, designation, initials } = data;
  await pool.query(`UPDATE public.users SET name = $1, designation = $2, initials = $3 WHERE identifier = $4;`, [
    name.trim(), designation.trim(), initials.trim().toUpperCase(), identifier.trim().toLowerCase()
  ]);
}
